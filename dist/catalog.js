...
var fallbackByokPresets = [
  ...
  { id: "anthropic", label: "Anthropic", description: "Claude через Messages API", apiFormat: "anthropic", baseUrl: "https://api.anthropic.com/v1", modelId: "claude-sonnet-4-6", contextWindow: 1e6, maxInputTokens: null, maxOutputTokens: 128000, inputPricePerMillionUsd: 3, cacheReadPricePerMillionUsd: 0.3, cacheWritePricePerMillionUsd: 3.75, outputPricePerMillionUsd: 15, inputTypes: ["text", "image", "pdf"], outputTypes: ["text"], catalogProviderId: "anthropic", catalogModelId: "claude-sonnet-4-6" },
  { id: "openai", label: "OpenAI", description: "GPT через Responses API", apiFormat: "responses", baseUrl: "https://api.openai.com/v1", modelId: "gpt-5.5-pro", contextWindow: 1050000, maxInputTokens: 922000, maxOutputTokens: 128000, inputPricePerMillionUsd: 30, cacheReadPricePerMillionUsd: null, cacheWritePricePerMillionUsd: null, outputPricePerMillionUsd: 180, inputTypes: ["text", "image", "pdf"], outputTypes: ["text"], note: "Нужен API key платформы OpenAI; подписка ChatGPT не является API key.", catalogProviderId: "openai", catalogModelId: "gpt-5.5-pro" },
...
function calculateUsageCostUsd(usage, prices) {
  if (usage.outputTokens == null || prices.inputPricePerMillionUsd == null || prices.outputPricePerMillionUsd == null)
    return null;
  ...
  if (cacheRead > 0 && prices.cacheReadPricePerMillionUsd == null)
    return null;
  if (cacheWrite > 0 && prices.cacheWritePricePerMillionUsd == null)
    return null;
  return uncached / 1e6 * prices.inputPricePerMillionUsd + cacheRead / 1e6 * (prices.cacheReadPricePerMillionUsd ?? 0) + cacheWrite / 1e6 * (prices.cacheWritePricePerMillionUsd ?? 0) + Math.max(0, usage.outputTokens) / 1e6 * prices.outputPricePerMillionUsd;
}
