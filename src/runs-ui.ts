...
export function renderRunsTable(runs: ByokLedgerRecord[], totals?: ByokLedgerTotals, options: { emptyHint?: string; selectedId?: string } = {}): string {
 ...
 * Row click emits `byok-run-open` with the record and marks it selected. */
export function defineByokRunsView(customElements?: CustomElementRegistry): void {
  ...
  class ByokRunsView extends (typeof HTMLElement !== 'undefined' ? HTMLElement : (class {} as unknown as typeof HTMLElement)) implements RunsElement {
    ...
    private onClick = (event: MouseEvent) => {
      const row = (event.target as HTMLElement | null)?.closest?.('[data-byok-run-id]') as HTMLElement | null
      if (!row) return
    ...
    private currentRuns(): ByokLedgerRecord[] {
      try { const value = JSON.parse(this.runs); return Array.isArray(value) ? value : [] } catch { return [] }
    }
    private render() {
